...
# Gets the emoji for a level name (internal)
...
_log_message() {
    ...
    if [[ "$CONSOLE_LOG" == "true" ]]; then
        local console_emoji
        console_emoji=$(_get_log_level_emoji "$level_name")
        local console_log_entry
        console_log_entry=$(_format_log_message "$level_name" "${console_emoji:+$console_emoji }$console_message")
        _log_to_console "$console_log_entry" "$level_name" "$level_value"
    fi
